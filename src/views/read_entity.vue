  ...
  <div class="content">
    ...
    <div class="search-wrapper">
      <input
        type="text"
        v-model="search"
        placeholder="Search entity by product name or .."
      />
      <br />
      <p v-if="products.length === 0">No product found in your search</p>
      <p v-else-if="products.length === 1">
        Your search found {{ products.length }} product
      </p>
      <p v-else>
        Your search found {{ products.length }} products of
        {{ productList.length }}
      </p>
      <!-- Nav pills -->
      <ul class="nav nav-pills justify-content-center" role="tablist">
        ...
        <li class="nav-item">
          <a class="nav-link active" data-toggle="pill" href="#product"
            >Product</a
          >
        </li>
        <li class="nav-item">
        ...
        <div id="product" class="container tab-pane active">
          ...
          <table class="table" v-for="product in products" :key="product.id">
            ...
            <tr>
              ...
              <th>{{ product.introducedOn }}</th>
            </tr>
            <tr>
              <td>Retail Price:</td>
              <th>{{ product.retailPrice }}</th>
            </tr>
            <tr>
              <td>Units Purchased:</td>
              <th>{{ product.unitsPurchased }}</th>
            </tr>
            <tr>
              <td>Value Purchased:</td>
              <th>{{ product.valuePurchased }}</th>
            </tr>
            <tr>
              <td>Units Sold:</td>
              <th>{{ product.unitsSold }}</th>
            </tr>
            <tr>
              <td>Value Sold:</td>
              <th>{{ product.valueSold }}</th>
            </tr>
            <tr>
              <td>Units Instock:</td>
            ...
            <tr>
              <td>Value Instock:</td>
              <th>{{ product.valueInstock }}</th>
            </tr>
            <tr>
              <td>Sales Cost:</td>
              <th>{{ product.salesCost }}</th>
            </tr>
            <tr>
              <td>Gross Profit:</td>
              <th>{{ product.grossProfit }}</th>
            </tr>
            <tr>
              <td>Discontinue:</td>
              <th>{{ product.discontinue }}</th>
            </tr>
            <tr>
              <td></td>
              <th>
                <!-- edit product -->
                <router-link
                  :to="{ name: 'edit_entity', params: { id: product.id } }"
                  class="edit"
                >
                  Edit
                </router-link>
...
export default {
  ...
  computed: {
    // FILTER PRODUCTS (search by name or checkin)
    products() {
      ...
      return this.productList.filter(
        (product) => product.productName
          ...
          || product.source.toLowerCase().includes(this.search.toLowerCase()),
      );
    },
  },
  created() {
  ...
  methods: {
    ...
    async deleteProduct(id) {
      const endpoint = `/delete-products/${id}`;
      try {
        await axios.get(api + endpoint).then(() => {
          // eslint-disable-next-line no-underscore-dangle
          this.productList.splice(
            // eslint-disable-next-line no-underscore-dangle
            this.productList.findIndex((i) => i._id === id),
            1,
          );
        });
